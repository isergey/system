...
-->
    <xsl:if test="explain/indexInfo/index[@scan='true']">
      <xsl:choose>
        <xsl:when test="$scan.to.form">
         <xsl:element name="input">
           ...
           <xsl:attribute name="onclick">scan('')</xsl:attribute>
         </xsl:element>
        </xsl:when>
        <xsl:otherwise>
