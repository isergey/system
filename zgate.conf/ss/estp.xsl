...
<xsl:template match="creationDateTime">
  <tr>
  <td class="label"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='LBL_REQUEST_DAT']"/></td>
  <td class="data">
...
<xsl:template match="taskSpecificParameters/taskPackage/targetPart/actualQuery | taskSpecificParameters/taskPackage/targetPart/query">
  <!--<xsl:if test="$fmt='F'">-->
    ...
    <td class="data"><div class="request"><xsl:value-of select="."/></div></td>
    </tr>
  <!--</xsl:if>-->
</xsl:template>
