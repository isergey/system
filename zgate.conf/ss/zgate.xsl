            ...
            <xsl:element name="a">
              <xsl:choose>
                <xsl:when test="$scan.to.form">
                  <xsl:attribute name="href">javascript:setForm('<xsl:value-of select="translate(term, '\', ' ')"/>')</xsl:attribute>
                </xsl:when>
                <xsl:otherwise>
...
<!-- PQ, PQS -->
    <xsl:if test="(@type='SearchResponse' or @type='PresentResponse') and not (database='IR-Extend-1' or database=$circ.db)">
      <xsl:text> [ </xsl:text>
        <a href="{$cgi.script.URL}?ACTION=pq&amp;SESSION_ID={$session.id}&amp;DB={database}&amp;TERM={term}&amp;LANG={$lang}" class="save_request_link"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_FORM_PQ']"/></a>
      <xsl:text> ] </xsl:text>
<!--
      <xsl:text> [ </xsl:text>
    ...
    <xsl:choose>
      <xsl:when test="@type='SearchResponse' or @type='PresentResponse'">
        <xsl:if test="$start &gt; 1">
          ...
          <xsl:choose>
            <xsl:when test="$fmt = 'B'">
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+1+{$max.records}+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>|&lt;</xsl:text></a>
            </xsl:when>
            <xsl:otherwise>
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+1+1+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>|&lt;</xsl:text></a>
            </xsl:otherwise>
          </xsl:choose>
          <xsl:text> ]</xsl:text>
          <xsl:text> [ </xsl:text>
    ...
    <xsl:choose>
      <xsl:when test="@type='SearchResponse' or @type='PresentResponse'">
        <xsl:if test="$start + count(record) - 1 &lt; resultCount">
          ...
          <xsl:choose>
            <xsl:when test="$fmt = 'B'">
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{resultCount - $max.records + 1}+{$max.records}+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>&gt;|</xsl:text></a>
            </xsl:when>
            <xsl:otherwise>
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{resultCount}+1+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>&gt;|</xsl:text></a>
            </xsl:otherwise>
          </xsl:choose>
          <xsl:text> ]</xsl:text>
        </xsl:if>
...
<!-- TASK PACKAGE -->
      <xsl:if test="(@syntax='1.2.840.10003.5.106' or @syntax='1.2.840.10003.5.109.10') and (eSTaskPackage/packageType='1.2.840.10003.9.2' or eSTaskPackage/packageType='2')">
