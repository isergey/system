...
<xsl:template name="record.rusmarc">
  <xsl:choose>
    ...
    <xsl:otherwise>
      ...
      <xsl:variable name="have.class" select="field[@id='675' or @id='676' or @id='680' or @id='686']"/>
      <xsl:choose>
      ...
      <xsl:if test="$fmt='F'">
        ...
        <xsl:if test="$record.source or not($type='x' or $type='y' or $type='z') and $class and $have.class">
          <table width="100%">
          <tr></tr><tr>
          ...
          <td align="right" valign="top">
            <xsl:if test="not($type='x' or $type='y' or $type='z') and $class and $have.class">
        ...
        <xsl:if test="$holdings and count(../../holdingsData) = 0">
          <div class="holders">
          <xsl:text>Документ есть в следующих библиотеках:</xsl:text><br/>
...
<xsl:template name="subjects">
  ...
  <xsl:variable name="p615" select="count(field[@id='615'])"/>

  <xsl:if test="$p600 + $p601 + $p602 + $p605 + $p606 + $p607 + $p610 + $p615 &gt; 0">
  <div class="subjects">
...
-->
    ...
    <xsl:text>.</xsl:text>
    <xsl:if test="not($inline.subjects)">
      <br/>
    </xsl:if>
  </xsl:for-each>
  ...
  <xsl:for-each select="field[@id='615']">
    <xsl:text> </xsl:text>
    <xsl:value-of select="$p600 + $p601 + $p602 + $p605 + $p606 + $p607 + $p610 + position()"/>
    <xsl:text>. </xsl:text>
    <xsl:choose>
      <xsl:when test="$follow.subject">
        <a href="{$cgi.script.URL}?ACTION=follow&amp;SESSION_ID={$session.id}&amp;TERM={subfield[@id='a']}[1,21]&amp;LANG={$lang}">
