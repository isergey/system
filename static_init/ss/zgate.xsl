...
<!-- ORDER -->
      <xsl:if test="@syntax !='1.2.840.10003.5.106' and @syntax !='Extended'">
        <xsl:if test="$item.order">
        <span class="oitem">
        <a href="/ajax/document.html?" onmouseover="appendBD(this)" class="thickbox"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_PREORDER_ITEM']"/></a>
        </span>
        </xsl:if>
        <span class="ocopy">
        <a href="/ajax/copy.html?" onmouseover="appendBD(this)" class="thickbox"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_PREORDER_COPY']"/></a>
        </span>
      </xsl:if>
