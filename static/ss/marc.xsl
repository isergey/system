...
<xsl:template name="org.by.code">
  ...
  <a href="/ajax/reserved.html?code={$oname}&amp;name={$name}" class="thickbox" onmouseover="appendBD(this)">
    <img src="/templates/ksob_new/css/book_icon_open.png" alt="PNG image" title="{$msg/messages/localization[@language=$lang]/msg[@id='I_SEE_MAP']}"/>
  </a>
  <xsl:text> </xsl:text>
  <a href="/participants/?code={$oname}" target="_blank" class="map">
...
<xsl:template name="iso">
  <xsl:param name="sf.dlm">&#x201F;</xsl:param>
  <xsl:param name="f.dlm">&#x201E;</xsl:param>
  <xsl:param name="rec.term">&#x201D;</xsl:param>
<!-- Leader -->
...
<xsl:value-of select="$f.dlm"/>
<!-- Data -->
  <xsl:for-each select="field">
    <xsl:if test="indicator">
      <xsl:value-of select="indicator[@id='1']"/>
      <xsl:value-of select="indicator[@id='2']"/>
    </xsl:if>
    <xsl:for-each select="subfield">
      <xsl:value-of select="$sf.dlm"/>
      <xsl:value-of select="@id"/>
      <xsl:value-of select="."/>
    </xsl:for-each>
    <xsl:if test="not(subfield)">
      <xsl:value-of select="."/>
    </xsl:if>
    <xsl:value-of select="$f.dlm"/>
  </xsl:for-each>
<xsl:value-of select="$rec.term"/>
</xsl:template>
